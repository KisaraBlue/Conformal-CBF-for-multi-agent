...
# Fix: missing annotations in ynet_additional_files (1/2)
import shutil

#def load_SDD(scene, path='./data/ynet_additional_files/data/SDD/', video_path='/checkpoints/aa/SDD/videos/', mode='train', load_frames=False):
def load_SDD(scene, path='./data/ynet_additional_files/data/SDD/', video_path='./data/SDD/videos/', mode='train', load_frames=False):
    '''
    ...
    reference_image = get_reference_image(scene, path=path, mode=mode)

    path = os.path.join(path, mode)
    scenes = os.listdir(path)
    SDD_cols = ['trackId', 'xmin', 'ymin', 'xmax', 'ymax', 'frame', 'lost', 'occluded', 'generated', 'label']

    if load_frames is not False:
        print(f"Parsing scene: {scene}")
        video_filename = video_path + scene.split('_')[0] + '/video' + scene.split('_')[1] + '/video.mov'
        vid = cv2.VideoCapture(video_filename)
        if isinstance(load_frames, (list, np.ndarray)): # Efficient read
            frames = []
            for i in tqdm(load_frames):
                try:
    ...
    # Fix: missing annotations in ynet_additional_files (2/2)
    if not os.path.isfile(scene_path):
        annotation_filename = './data/SDD/annotations/' + scene.split('_')[0] + '/video' + scene.split('_')[1] + '/annotations.txt'
        shutil.copyfile(annotation_filename, scene_path)
    ...
    rec_trackId2metaId = {}
    for i, j in enumerate(scene_df['rec&trackId'].unique()):
        rec_trackId2metaId[j] = i
    scene_df['metaId'] = [rec_trackId2metaId[i] for i in scene_df['rec&trackId']]
    scene_df = scene_df.drop(columns=['rec&trackId'])
    return frames, scene_df, reference_image

def load_SDD_old(scene='nexus_4', path='./ynet_additional_files/data/SDD/', video_path='/checkpoints/aa/SDD/videos/', mode='train'):
    '''
    Loads data from Stanford Drone Dataset. Makes the following preprocessing:
    -filter out unnecessary columns (e.g. generated, label, occluded)
    -filter out non-pedestrian
...
def get_reference_image(scene, path='./data/ynet_additional_files/data/SDD/', mode='train'):
    # Get reference image
    reference_image = cv2.imread(os.path.join(path, mode, scene, 'reference.jpg'))
    # Preprocess images, in particular resize, pad and normalize as semantic segmentation backbone requires
    # Resize
    factor=0.25
    reference_image = cv2.resize(reference_image, (0,0), fx=factor, fy=factor, interpolation=cv2.INTER_AREA)
    # Pad
    H, W, C = reference_image.shape
    downsample_factor = 2**5
    H_new = int(np.ceil(H / downsample_factor) * downsample_factor)
    W_new = int(np.ceil(W / downsample_factor) * downsample_factor)
    reference_image = cv2.copyMakeBorder(reference_image, 0, H_new - H, 0, W_new - W, cv2.BORDER_CONSTANT)
    # Normalize
    preprocessing_fn = smp.encoders.get_preprocessing_fn('resnet101', 'imagenet')
    reference_image = preprocessing_fn(reference_image)
    reference_image = reference_image.transpose(2, 0, 1).astype('float32')
...
def load_and_window_SDD(step=1, window_size=30, stride=None, path=None, mode='train', pickle_path=None):
    """

...
def point_to_poly(point, rad):
    x, y = point[0], point[1]
    return pc.box2poly(np.array([[x - rad, y - rad], [x - rad, y - rad]]))

# finish this if needed:

class RRT_3D:
    """
    Class for 3D RRT planning
    """
    ...
    def __init__(self, start, goal, obstacle_list, rand_area,
        ...
        start:Start Position [x,y,th]
        goal:Goal Position [x,y,th]
        obstacleList:obstacle Positions [[x,y,size],...]
        randArea:Random Sampling Area [min,max]
